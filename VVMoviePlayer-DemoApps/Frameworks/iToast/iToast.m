...
- (void) show:(iToastType) type{
	...
	if (image) {
		v.frame = CGRectMake(0, 0, image.size.width + textSize.width + hzPad+5, MAX(textSize.height, image.size.height) + vtPad);
		label.center = CGPointMake(image.size.width + hzPad + (v.frame.size.width - image.size.width - hzPad) / 2, v.frame.size.height / 2);
	} else {
		v.frame = CGRectMake(0, 0, textSize.width + hzPad, textSize.height + vtPad);
